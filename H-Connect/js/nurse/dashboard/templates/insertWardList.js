...
const $ward_selectBox_selected_el = $('.nurse.dashboard .taget_select .select_ward .ward_label');
...
function _dashboard_wardSelectBoxList_insert(_ward_list){
    if(_ward_list && _ward_list.length > 0){
        const _$ward_selectBox_listWrap = $('.nurse.dashboard .taget_select .select_ward .ward_option');
...
function _dashboard_sickRoom_insert(_ward){
    ...
    if(_ward){
        ...
        if(sickRoomList){
            ...
            const _$sickRoom_wardListEls = $('.nurse.dashboard .taget_select .select_inner .sickRoom_block');
            _$sickRoom_wardListEls.find('.ward_count').off().on('click', function(){
                $(this).next().toggle();
            });
            _$sickRoom_wardListEls.find('.ward_count .input_wrap input').off().on('change', function(){
                const _inputPatientEl = $(this).closest('.sickRoom_block').find('.patient_info input');  
                _inputPatientEl.attr('checked', !_inputPatientEl.attr('checked'));
            });
        }
    }
}
...
function _dashboard_firstWard_addEvent(){
    ...
    $('.nurse.dashboard .ward_option .ward_list').off().on('click', function(){
        // 설렉트 박스 선택 이벤트
