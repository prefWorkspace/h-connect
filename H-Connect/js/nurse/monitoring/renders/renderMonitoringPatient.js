const { monitorBlock_have, monitorBlock_none } = await import(
    importVersion('/H-Connect/js/nurse/monitoring/templates/monitoringPatientTml.js')
    );
const {
    selectSickBed, selectMeasurementInfoList, selectMeasurementInfoBioDataPage
} = await import(
...
const { addMonitoringEmptyBedClickEvent } = await import(
    importVersion('/H-Connect/js/nurse/monitoring/renders/addMonitoringEmptyBed.js')
    );
...
async function renderMonitoringPatientList() {
    ...
    const { measurementInfoSimpleBioDataList: patientList } = await selectMeasurementInfoBioDataPage();
