...
const { selectSickBed, selectMeasurementInfoList, selectSickRoom } = await import(
    importVersion('/H-Connect/js/nurse/monitoring/actions/monitoringAPI.js')
    );
const { updateMonitoringPatientItem } = await import(
    importVersion('/H-Connect/js/nurse/monitoring/renders/updateMonitoringPatient.js')
    );
    ...
    window.client.connect(headers, function() {
        ...
        client.addSubscribe('event', '/topic/public/event', function(res) {
            if (res) {
                ...
                updateMonitorRoomBlock(sickRoomList.filter(room => room.sickRoomCode === item.sickRoomCode)?.[0], patientList);
                // measurementInfo 체크
                if (data.eventType === 10 && data.measurementInfo.name && data.measurementInfo.patientCode) {
                    ...
                    client.addSubscribe('bioSignalSimpleData_' + data.measurementInfo.measurementCode, `/topic/public/bioSignalSimpleData/${data.measurementInfo.measurementCode}`, function(res) {
                        updateMonitorRoomBlock(sickRoomList.filter(room => room.sickRoomCode === item.sickRoomCode)?.[0], patientList.map(patient => patient.measurementCode === item.measurementCode ? { ...item, ...patientDataUpdate(res) } : patient));
                    });
                }
            }
        });
        ...
        // 모니터링 리스트 구독 추가
        patientList.map(item => client.addSubscribe('bioSignalSimpleData_' + item.measurementCode, `/topic/public/bioSignalSimpleData/${item.measurementCode}`, function(res) {
            updateMonitorRoomBlock(sickRoomList.filter(room => room.sickRoomCode === item.sickRoomCode)?.[0], patientList.map(patient => patient.measurementCode === item.measurementCode ? { ...item, ...patientDataUpdate(res) } : patient));
        }));
    }, function(error) {
        console.log(error);
