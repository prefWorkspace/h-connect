...
function new_SickBed_selectBox_handle(_wardList){
    ...
    $(".section.new_hospital .hospital_patient .selectBox2.s_select .optionList.ward_option2 .ward_list2").on("click", function(){
        ...
        const index = $(this).data("index"); //선택한 _wardList의 인덱스
        ...
        for(let i = 0; i < _wardList[index].sickRoomList.length; i++){
            total_bed_count = _wardList[index].sickRoomList[i].sickBedList.length;
            patient_count = _wardList[index].sickRoomList[i].roomPatientCount;
            new_sickRoomList += `
                <li class="optionItem room_list2" data-index=${i} data-sickroomcode="${_wardList[index].sickRoomList[i].sickRoomCode}">
                    <p>
                        <span>${_wardList[index].sickRoomList[i].sickRoom}</span> 호실
                    </p>
        ...
        $(".section.new_hospital .hospital_patient .selectBox2.s_select .optionList.room_option2 .optionItem.room_list2").on("click", function(){
            const target = $(this);
            new_sickBed_selectBox_handle(_wardList[index].sickRoomList, target);
        }) //리스트 정렬후 다시 이벤트 
    ...
    // $(".section.new_hospital .hospital_patient .selectBox2.s_select .optionList.room_option2 .optionItem.room_list2").on("click",new_sickBed_selectBox_handle) 
}

// 측정관리 신규 병상등록 병실 셀렉트 박스
function new_sickBed_selectBox_handle(sickRoomList, target){
